...
## B 类体力门槛失败结局的写作约定（2026-09-21 落地 5 处）

- **判定标准（用户拍板）**：`strength >= N` 门槛战斗的失败结局，**只给"被动迎战"的情节**拆出体力专属结局，委婉暗示玩家是没力气才打不过；**玩家主动攻击的情节不拆**（玩家会觉得是自己作死，怪不到体力门槛）。受伤/死亡的不统一**暂不改**。
- **实现方式**：新增结局节点、ID 加 `-力竭` 后缀，**只改体力分支的 elseScene 指向，原结局节点一律不动**。原因：多个结局是**共享节点**（`结局-被丧尸扑倒咬死` 源码注释即写明"覆盖所有汇入此处的战斗失败/QTE超时"，樱桃苑十几处 QTE 超时都指向它），直接改文案会污染非体力来源的死亡。**结局显示名与原结局保持一致**，暗示只走文案、不走结局名。**原结局若因此变成无引用的孤儿，直接删掉该节点定义**（用户偏好，不留冗余）。
- **文案手法**：用身体失控间接点出没力气（"膝盖像灌了铅" / "手臂在发抖，使不上力" / "一脚轻飘飘的" / "大腿酸得发软"），**不直说"体力不足"**；须避开 AI 雷同表述禁增清单。
- **已改 5 处**：装睡的丧尸→`结局-背后的偷袭-力竭`；地下车库的丧尸(3 分支)→`结局-被丧尸扑倒咬死-力竭`；图书馆-阅览室-惊醒→`结局-图书馆-书堆-力竭`；安盛街-试衣间→`结局-安盛街-试衣间丧尸扑脸-力竭`；安盛街-被包围(仅"硬闯")→`结局-安盛街-被尸潮吞没-力竭`。
- **明确不改**：主动攻击类（图书馆-大厅、益丰柜台后、安盛街收银台、公交站开门）；文案已暗示体力的（地铁站-站厅层/楼梯/滑扶手、图书馆藏书区扭打、安检区斧子太重）；非战斗的（张江-华大-风淋舱-击退，风淋舱互锁）。
- 审计工具 `tools/combat_penalty_report.js` 可复跑，B 类表自动反映 elseScene 指向。
