...
- **⚠ 在 core.js `_variables` 增删行 → `tools/stamina_report.py` 的 KNOWN_SITES core.js 区间整体平移，必须同步校正**（并同步 `--selftest` 的 mock 行号）。校正要**按内容重新定位**（搜 `id: "starvation"` / 场景名），不要盲目 +N——旧区间本身可能早已漂移。
