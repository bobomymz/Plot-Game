...
#### 续玩旧档 / 回溯也要兜住（2026-09-24 起引擎自动兜）

同一个 `ReferenceError` 还有**第二个来源**：变量注册了，但玩家手上的状态是「加变量之前」存下的。
`initGameState()` 的键集来自 `_variables`，而 `loadSave()` / `backtrack()` 是**直接接管旧状态、
不经过 initGameState** —— 旧档里没有新键，`new Function` 一样抛 `ReferenceError`。
症状特征：**只有字符串条件报错**（函数式条件写 `vars.xxx` 只是安静地拿到 `undefined`，
所以"只有一条报错"是假象，别当成单点 bug）。

引擎已在三条恢复路径自动兜底（`engine.js`）：

| 补什么 | 怎么补 | 调用点 |
| ----- | ----- | ----- |
| `_variables` 里新增的键 | `fillMissingDefaults()` 填初始值 | `applySave()`（`gameState` + `historyStack` 每一项）、`backtrack()` |
| `_reactive.computed` 里新增的派生键 | `refreshComputed()` 重算（派生量没有"初值"可填） | 同上 |

⚠ 恢复路径上**只重算派生量，不要跑整个 `applyReactive()`** —— 那会把饥饿/疲劳规则提前到
读档瞬间结算（玩家什么都没做就掉体力）。规则仍按原流程在第一次 `applyEffect` 时跑。

所以**新增变量不需要再手动 `SAVE_VERSION + 1`**；只有**改字段含义/结构**（补默认值反而错）时才 bump 弃档。
回归：`node tools/save_compat_selftest.js`（27 断言，含"不补必炸"的对照组）。详见 `tools/旧存档兼容性修复报告.md`。
...
- **照明分级**（强弱光源是明文设定，不是遗漏）：**手电筒 `hasTorch` = 强光源**，全域照明判定可用（`canSee`、金谊B2、五金店、全家员工通道等伸手不见五指级黑暗都认它）。**手机 `hasPhone && phoneBattery > 0` = 弱光源**，只在短通道级黑暗可用（菜市场员工通道、仁济坡道入口），且看不清细节（车库接线图、F区车辆描述）。火把 `hasFireTorch` 仅建平车库深处认。写新黑暗场景时先决定它是"强黑暗"（只认手电）还是"弱黑暗"（手机也算），手机参与照明/扫码/WiFi 时记得 `add: { phoneBattery: -5 }`，电量归零后文本写"屏幕黑了"。手机无充电途径，电量耗尽即终局。**手机有 2+ 处拾取点时，必须同步写 `_phoneOrigin`**（`"own"`=全家便利店妈妈遗物原机，有同学微信；`"store"`=新达汇华为展示机）——复旦江湾章 b 链（打电话叫同学来帮忙）以 `_phoneOrigin == "own"` 为门控，新增拾取点漏写则该链**静默失效**（不报错、只是选项永不出现）。
...
- 尽量从玩家视角进行测试，逐个节点跳转（grep），不要一次性读取整个文档。
