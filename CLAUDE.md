...
```javascript
"场景ID": {
  image: "images/path.png",               // 场景图片路径，也支持函数(vars) => string
  text: "剧情文字，支持 {变量名} 插值",       // 字符串或函数(vars) => string，注意语法，不能直接包含引号，不要连续写"\n\n"
  style: "color: red; font-size: 18px;",  // 可选，直接作用在文本容器上
  onEnter: { set: {...}, add: {...} },   // 进入时触发的效果（可选）
  choices: [ ... ]                       // 选项列表（可选，无选项则剧情结束）
}
```
...
```javascript
{
  text: "选项文字",               // 字符串或函数(vars) => string，注意语法，不能直接包含引号，不要连续写"\n\n"
  nextScene: "目标场景ID",        // 点击后跳转
  effect: { set: {...} },       // 点击时触发效果（可选）
  condition: "表达式",           // 显示条件（可选）
  elseScene: "不满足时跳转",      // condition 不满足时显示的替代选项（可选）
