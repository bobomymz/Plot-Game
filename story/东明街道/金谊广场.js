...
角落里放着一个保温杯，杯身上印的字已经磨得快看不清了——只剩一个模糊的“加油”和一颗褪色的红心。\n这是一家曾经有人认真经营过的店。",
  ...
  "金谊广场-停车场-救完": {
    ...
    choices: [
      { text: "收下地图",
        effect: updateTime(2, { set: { hasMap: true, _hasPoliceMap: true }, add: { itemCount: 1 } }),
        nextScene: "金谊广场-地面停车场" }
    ]
  },
  ...
  "金谊广场-停车场-搜刮": {
    ...
    text: function(vars) {
      if (vars._visit['金谊广场-停车场-搜刮'] > 1) {
        ...
        if (!vars.hasBottle) again += "\n老桑塔纳的后备箱还敞着——里面那半瓶矿泉水，你可以顺手拿走。";
        again += "\n河边丧尸的喉音似乎比刚才更近了。你不想在这里多待。";
        return again;
      }
      return "你在车辆之间穿梭，试着拉了几扇车门。大部分都锁着，只有一辆老款桑塔纳的后备箱没锁——里面只有半瓶矿泉水和一件发霉的雨衣。\n\
    ...
    choices: function(vars) {
      ...
      if (!vars.hasBottle) {
        cs.push({
          ...
          nextScene: "金谊广场-停车场-搜刮-拿到水",
          effect: { set: { hasBottle: true, bottleWater: 1, waterToxic: false }, add: { itemCount: 1 } },
          elseScene: "整理整理"
        });
      }
      cs.push({ text: "继续", nextScene: "金谊广场-地面停车场", effect: updateTime(1) });
      return cs;
    }
  },
...
死路。空气中那股甜腥味似乎更浓了。你花了一些时间才摸回车库入口。";
    },
    choices: [
  ...
  "金谊广场-B2货梯间": {
    ...
    text: function(vars) {
      ...
      if (vars.hasGasMask && vars.maskRemainingUses > 0) {
        desc += "你戴上了防毒面具——活性炭滤层隔绝了那股甜腥味，呼吸变得安全了。\n货梯的按钮还亮着，似乎还能用。";
      } else {
        desc += "你感到一阵头晕——那股甜腥味越来越浓了。你的喉咙开始发紧，视野边缘在变暗。\n";
  ...
  "金谊广场-3F-幸存者": {
    ...
    text: function(vars) {
      ...
      var desc = "你循着声音走进一家夸父炸串店。板前座位后面，一个年轻男人蹲在吧台下面，手上拿着一根竹签。\n";
      desc += "他看到你，先是缩了一下，然后慢慢放松了肩膀。\n";
      desc += "“你是……活人？”他的声音沙哑，像是很久没跟人说过话了。\n";
      desc += "他叫小林——二十四五岁，原来在隔壁京东电器做销售。疫情爆发那天他正在夸父炸串店吃鸡肉串，然后就再也没出去过。\n";
      desc += "他没有加入长廊那群人。他说他不信任人多的地方。\n";
      desc += "“人多的地方，最后都会出事。”他把竹签放在膝盖上，手指无意识地弹着竹签。\n";
      return desc;
    },
    choices: function(vars) {
      var choices = [];
  ...
  "金谊广场-3F-幸存者-聊长廊": {
    ...
    text: function(vars) {
      ...
      if (vars._jinyiSurvivorsRobbed) {
        ...
        desc += "“我离开长廊，就是因为那个人。”小林把竹签在膝盖上转了一圈。“老周说人人平等，但那个戴眼镜的不这么想。他觉得自己比别人更有资格活下去。你知道这种人最可怕的是什么吗——他们没错。从某种角度来说，他们确实更适合活到最后。”";
      }
  ...
  "金谊广场-B1童涵春堂": {
    ...
    text: function(vars) {
      ...
      desc += "中药柜的抽屉被拉开了一大半，草药撒了一地。西药区的货架倒是整齐。\n";
      if (!vars.hasMercuryPill) {
