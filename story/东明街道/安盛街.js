  ...
  "安盛街东侧": {
    ...
    text: function(vars) {
      ...
      if (!vars._visit["理发店内部"]) {
        desc = "你来到了安盛街东侧。小时候你经常来这里逛，那时人来人往、商铺林立。如今卷帘门半掩、招牌歪斜。\n路边有一家文具店，卷帘门半掩着。你曾经理发的那个小店就在前面不远——希望那里还安全。\n";
      } else {
    ...
    text: function(vars) {
      ...
      if (!vars._visit["理发店内部"]) { // 没来过
        ...
        if (isNight) {
          ...
          basicDes += "白天看起来和上次差不多——门窗紧闭，窗帘还拉着，门口旋转灯箱独自转着。";
        }
      }
  ...
  "理发店内部": {// 入口，从街上进入时触发
    ...
    text: function(vars) {
      ...
      if (vars.mercuryTier >= 1) {
        // 汞 20+ 皮肤灰白：理发店的镜子是少数"自视"载体
        return "你推门走进理发店，顺手把门带上。四把理发椅前的镜子里映出你——你停住了。那张脸比记忆里灰，像褪了色的照片。你抬手摸了摸脸颊，皮肤是凉的。镜子里的那个人也跟着抬起手。";
      }
...
柜门没锁，轻轻一拉就开了。里面整整齐齐码着一叠没拆吊牌的新衣服：深蓝夹克、灰格衬衫、一件小号的牛仔外套。最上面压着一张纸条，字迹潦草，一笔写成的。";
    },
    choices: [
      {
