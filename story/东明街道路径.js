...
你绕过挣扎的保安，退出了银行。",
  ...
  "银行-金库": {
    image: "images/小区周边/bankVault.png",
...
你把凭条叠好塞进口袋，虽然也不知道留着还有什么用。",
  ...
  // ==================== 联华超市 ====================
  "小超市": {
    image: "images/小区周边/lianhuaSupermarket.png",
  ...
  "联华超市-仓库": {
    image: "images/小区周边/supermarketWarehouse.png",
    text: "你推开仓库的门。里面堆着几箱饮料和一些滞销的零食。角落里有一扇活板门，上面焊着一根铁栓——通往地下室。你拉开铁栓，用手机屏幕的光照了照，下面是一个干燥的地下室，面积不大，但胜在隐蔽。\n万一晚上没地方去，这里也许能凑合一晚——至少比露宿街头强。",
    choices: [
      {
        text: "记下这个位置，离开",
        nextScene: "三林路"
  ...
  "小区东门-整装待发-躲藏": {
    ...
    text: function(vars) {
      if (vars._hideFail) return "你绕到保安亭的背面蹲下，但一群丧尸漫无目的地游荡过来，把保安亭围住了。它们没有发现你，但你也出不去了——你被困了十多分钟，最后只能趁它们稍微散开时冲出去。慌乱中你被一只丧尸抓了一下。";
      return "你绕到保安亭的背面，蹲在墙根和灌木丛之间的空隙里。保安亭的玻璃上映着远处丧尸摇晃的影子，但它们看不见你。\n你等了不知多久，外面的声音渐渐稀疏了。你探出头，街道上已经空了不少。";
    },
    choices: [
  ...
  "三林路-环林东路十字路口-躲藏": {
    ...
    text: function(vars) {
      if (vars._hideFail) return "你蹲在一辆被追尾的面包车后面，但一队丧尸挤挤挨挨地从车旁经过，其中一只撞到了侧视镜——啪地一声。它被声音吸引，摇摇晃晃绕到了车后……你只能一脚踹开它夺路而逃。";
      return "你蹲在一辆被追尾的面包车后面，透过破碎的车窗观察着路口。丧尸群在你刚才站的地方徘徊了一会儿，然后朝不同的方向散去了。你等了一刻钟，确认安全后才站起来。";
    },
    choices: [
  ...
  "三林路-躲藏": {
    ...
    text: function(vars) {
      if (vars._hideFail) return "你闪进一家门锁坏掉的小店门廊深处，缩在黑暗里。但今天运气不好——一队丧尸直接朝你这边涌来。它们不是发现了你，只是恰好走了这条路……狭小的门廊无处可躲，你只能冲出去。";
      return "你闪进一家门锁坏掉的小店门廊深处，缩在黑暗里。外面的脚步声来来回回，但没人注意到这个凹陷处。过了不知多久，声音远去了。你活动了一下发麻的腿，走了出来。街道安静了许多。";
    },
    choices: [
  ...
  "三林路-东明路十字路口-躲藏": {
    ...
    text: function(vars) {
      if (vars._hideFail) return "你蜷缩在路口配电箱和绿化带的夹缝里。但一只丧尸晃晃悠悠走到配电箱旁就不动了——它就一直站在那里。你等了很久它还是不走，夹缝里又闷又窄。你只能冒险冲出去。";
      return "你蜷缩在路口配电箱和绿化带的夹缝里，用枝叶盖住自己。一只丧尸走到配电箱旁，离你伸手可及，但它没有低头看。它站了一会儿，慢悠悠地走了。你等一切安静后才钻出来。";
    },
    choices: [
  ...
  "东明路-三林路-躲藏": {
    image: "images/小区周边/东明路-小区西门门口.png",
    onEnter: function(vars) {
      updateTime(15 + Math.floor(Math.random() * 16))(vars);
      if (vars.chasedByZombies >= 4 && Math.random() < 0.4) {
        vars.strength = Math.max(0, vars.strength - 1);
    ...
    text: function(vars) {
      if (vars._hideFail) return "你蹲在一棵粗大的法国梧桐后面，但尸群太密集了——它们像潮水一样涌过，树干根本挡不住你。你只能提前暴露，在丧尸反应过来之前冲出去。慌乱中你被一根树枝绊了一下，擦伤了手臂。";
      return "你蹲在一棵粗大的法国梧桐后面。树干的阴影把你完全吞没了。几只丧尸拖沓着经过，其中一只在树干上蹭了蹭背，然后慢悠悠地走开了。你屏住呼吸等它们走远，才从树后探出头。";
    },
    choices: [
      { text: "继续", nextScene: "东明路-三林路" }
    ]
  }
});