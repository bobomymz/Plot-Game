...
墙角的天花板上开着一个检修口，一截笼式爬梯垂下来，通往上面的设备夹层。";
    ...
    choices: function(vars) {
      ...
      cs.push({ text: "退回连廊", nextScene: "张江-华大-连廊", effect: updateTime(1) });
      return cs;
    }
  },
    ...
    choices: function(vars) {
      if (vars._jinbaoLeft) {
        return [
          ...
          { text: "退回灰区", nextScene: "张江-华大-灰区", effect: updateTime(1) }
        ];
      }
      var cs = [];
...
走廊一头是风淋舱，另一头的双开气密门虚掩着，门上的牌子：核心工艺区。侧面还有一扇窄些的门：辅助运维区。";
    ...
    choices: [
      ...
      { text: "回风淋舱那头", nextScene: "张江-华大-风淋舱", effect: updateTime(2) }
    ]
  },
...
没有人知道你对他做了什么。这间车间里也不会有人再喊他名字了。";
      ...
      if (vars._fabFigBDone) {
        return "老陈回到他的设备边上，继续盯着沉积速率的曲线，笔尖在记录本上一下一下地顿。\n\
他朝你这边点了点头——不多说话，但也不躲了。";
      }
      if (vars._jinbaoLeft) {
...
这里没有光，就走不了。",
    choices: [
      { text: "退回灰区", nextScene: "张江-华大-灰区", effect: updateTime(2) }
    ]
  },
...
格栅的缝隙里漏着下方的微光——透过这些缝，能看见整个白区在脚底下铺开。";
    ...
    choices: [
      ...
      { text: "原路退回灰区", nextScene: "张江-华大-灰区", effect: updateTime(4) }
    ]
  },
...
“我爸呢。”";
  ...
  "张江-华大-动力站-告知": {
    ...
    text: function(vars) {
      var segs = [];
      segs.push("“安居苑 8 号楼 204。”你说，“你爸他——”\n后面的话你尽量说得平。他从哪天开始不对劲，怎么一个人撑着，最后怎么样了。");
      segs.push("洪金宝一直站着听，一动不动。听到最后，他扶住仪表台，慢慢地、慢慢地坐了下去，像一栋被抽掉承重柱的楼。\n他没有嚎。他只是用手背抵着嘴，肩膀一耸一耸，从喉咙里漏出一点压碎的声音。");
...
他的皮肤在昏暗里泛着暗灰的金属光，浑浊的眼球锁定你，全是攻击性。这具身体生前经年累月碰的那些东西，让它死了以后还这么硬。";
  ...
  "张江-检测中心-制服": {
    ...
    text: function(vars) {
      var segs = [];
      segs.push("你把他掀翻在仪器桌前，抄起设备的连接线缠住他的胳膊，又把白大褂的袖子在身后打了个死结。他挣扎的力道大得吓人，缠了很久才不动了——嘴里还在念。\n“011……出了……签……”");
      segs.push("你蹲在地上喘了半天，才想起他手里那几页纸。\n纸从他僵硬的手指里抽出来的时候，发出很轻的、纤维撕裂的响。");
